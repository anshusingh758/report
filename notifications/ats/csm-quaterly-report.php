<?php
	...
	$lastMondayYMD = date("Y-m-d",strtotime('last Monday'));
	$lastSundayYMD = date("Y-m-d",strtotime('last Sunday'));
	$titleDate = date("m_d",strtotime('last Sunday'));
	...
	if(mysqli_num_rows($managerNameQUERY) > 0){
		while($managerNameROW = mysqli_fetch_array($managerNameQUERY)){
			...
			// Email subject
			$mail->Subject = 'Weekly Report_'.$managerNameROW['managerName'].'_'.$titleDate;
			...
			<body>
				<table style="width: 100%;">
					...
					<tr>
						<td style="font-size: 20px;font-weight: bold;">Weekly Report by Recruiter<span style="color: #2266AA;"> ('.$managerNameROW["managerName"].')</span><span style="font-size: 16px;color: #449D44;"> ('.$lastMonday.'<span style="color:#2266AA;"> --to-- </span>'.$lastSunday.')</span></td>
					</tr>
					<tr>
						<td><br>
			...
			while($recruiterListROW = mysqli_fetch_array($recruiterListQUERY)){
				...
				FROM
				(SELECT
					COUNT(CASE WHEN (date_format(date_created,'%Y-%m-%d') BETWEEN '$lastMondayYMD' AND '$lastSundayYMD') THEN joborder_id END) AS newA,
				...
				(SELECT
					...
					COUNT(DISTINCT CASE WHEN (date_format(job.date_created,'%Y-%m-%d') BETWEEN '$lastMondayYMD' AND '$lastSundayYMD') THEN job.joborder_id END) AS newAH
				FROM
					joborder AS job
				...
				AND
					date_format(cjsh.date,'%Y-%m-%d') BETWEEN '$lastMondayYMD' AND '$lastSundayYMD') AS c");
			...
			<body>
				<table style="width: 100%;">
					...
					<tr>
						<td style="font-size: 20px;font-weight: bold;">Weekly Report by Client<span style="color: #2266AA;"> ('.$managerNameROW["managerName"].')</span><span style="font-size: 16px;color: #449D44;"> ('.$lastMonday.'<span style="color:#2266AA;"> --to-- </span>'.$lastSunday.')</span></td>
					</tr>
					<tr>
						<td><br>
							<table style="width: 100%;border: 1px solid #ddd;">
			...
			while($clientListROW = mysqli_fetch_array($clientListQUERY)){
				...
				AND
					date_format(job.date_created,'%Y-%m-%d') BETWEEN '$lastMondayYMD' AND '$lastSundayYMD'
				...
				WHERE
					comp.company_id = '$clientId'
				AND
					date_format(cjsh.date,'%Y-%m-%d') BETWEEN '$lastMondayYMD' AND '$lastSundayYMD'
