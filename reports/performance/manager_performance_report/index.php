...
<body>
	...
	<section>
		<div class="container-fluid">
			<div class="row">
				<div class="col-md-12 report-title">Manager Performance Report</div>
			...
			<form action="index.php" method="post">
				...
				<div class="row main-section-row">
					<div class="col-md-4 col-md-offset-4">
						...
						<select id="manager-list" class="customized-selectbox-with-all" name="manager-list[]" multiple required>
							<?php
...
<?php
		...
		$managerData = "'".implode("', '",$_REQUEST['manager-list'])."'";
							...
							<?php
								...
								foreach ($fromDate as $fromDateKey => $fromDateValue) {
									...
									$mainQUERY = "SELECT
										...
										h.total_job,
										h.total_openings,
										i.total_unanswered_job,
										i.total_unanswered_job_openings,
										i.total_onhold_internal
									FROM
									(SELECT
									...
									FROM
										user AS u
									WHERE
										CONCAT(u.first_name,' ',u.last_name) IN ($managerData)
									...
									FROM
									    ...
									    JOIN user AS u ON comp.owner = u.user_id
									WHERE
										CONCAT(u.first_name,' ',u.last_name) IN ($managerData)
									AND
										date_format(cjsh.date, '%Y-%m-%d') BETWEEN '$startDate' AND '$endDate'
									GROUP BY u.user_id) AS b ON b.manager_id = a.manager_id
									LEFT OUTER JOIN
									(SELECT
										u.user_id AS manager_id,
									...
									AND
										date_format(cjsh.date, '%Y-%m-%d') BETWEEN '$startDate' AND '$endDate')
									GROUP BY u.user_id) AS c ON c.manager_id = a.manager_id
									LEFT OUTER JOIN
									(SELECT
										u.user_id AS manager_id,
									...
									AND
										date_format(cjsh.date, '%Y-%m-%d') BETWEEN '$startDate' AND '$endDate'
									GROUP BY u.user_id) AS d ON d.manager_id = a.manager_id
									LEFT OUTER JOIN
									(SELECT
										u.user_id AS manager_id,
									...
									FROM
										...
										JOIN company AS comp ON si.c_company_id = comp.company_id
									    JOIN user AS u ON comp.owner = u.user_id
									WHERE
										CONCAT(u.first_name,' ',u.last_name) IN ($managerData)
									AND
									...
									GROUP BY u.user_id) AS g ON g.manager_id = a.manager_id
									LEFT OUTER JOIN
									(SELECT
										u.user_id AS manager_id,
									...
									FROM
										cats.user AS u
										LEFT JOIN cats.company AS c ON c.owner = u.user_id
									...
									GROUP BY u.user_id) AS h ON h.manager_id = a.manager_id
									LEFT OUTER JOIN
									(SELECT
										u.user_id AS manager_id,
									    COUNT(DISTINCT job.joborder_id) AS total_unanswered_job,
									    SUM(job.openings) AS total_unanswered_job_openings,
									...
									AND
										job.status != 'Canceled'
									AND
										job.joborder_id NOT IN (SELECT cjsh.joborder_id FROM cats.candidate_joborder_status_history AS cjsh WHERE cjsh.joborder_id = job.joborder_id AND cjsh.status_to = '400' AND DATE_FORMAT(cjsh.date, '%Y-%m-%d') BETWEEN '$startDate' AND '$endDate')
									AND
										DATE_FORMAT(job.date_created, '%Y-%m-%d') BETWEEN '$startDate' AND '$endDate'
									GROUP BY u.user_id) AS i ON i.manager_id = a.manager_id
									WHERE
									...
									if (mysqli_num_rows($mainRESULT) > 0) {
										while ($mainROW = mysqli_fetch_array($mainRESULT)) {
											...
											$personnelType = "cs_manager";

											$dataPopup = "personnel_id=".$mainROW["manager_id"]."&personnel_name=".ucwords($mainROW["manager_name"])."&personnel_type=".$personnelType."&start_date=".$startDate."&end_date=".$endDate;

							?>
							<tr class="tbody-tr-style">
								<td><?php echo ucwords($mainROW["manager_name"]); ?></td>
							<?php if (isset($_REQUEST["customized-multiple-month"])) { ?>
