...
<?php
	if (isset($_REQUEST["form-submit-button"])) {
		$fromDate = $toDate = $mainTableData = $optimizationMatrixClientInfo = $dipAndDullClient = $optimizationMatrixColorMonth  = $countTheClientByColor = array();
		
		if (isset($_REQUEST["customized-multiple-month"])) {
			$monthsData = array_unique(explode(",", $_REQUEST["customized-multiple-month"]));
			sort($monthsData);
			$clength = count($monthsData);
			for($x = 0; $x < $clength; $x++) {
     			$monthsData[$x];
			}
			...
			sort($fromDate);
			$clength = count($fromDate);
			for($y = 0; $y < $clength; $y++) {
     			$fromDate[$y];
			}
			sort($toDate);
			$clength = count($toDate);
			for($z = 0; $z < $clength; $z++) {
     			$toDate[$z];
			}
		} else {
...
<?php
		...
		if (mysqli_num_rows($clientOptimizationMatrixQUERY) > 0) {
			while ($clientOptimizationMatrixROW = mysqli_fetch_array($clientOptimizationMatrixQUERY)) {
		    	...
		    	if ($clientOptimizationMatrixROW["margin_max"] == '') {
		    		$columnKey = $clientOptimizationMatrixROW["margin_min"]." > ";
		    	} else {
		    		$columnKey = $clientOptimizationMatrixROW["margin_min"]." - ".$clientOptimizationMatrixROW["margin_max"];
		    	}
    			
    			if ($clientOptimizationMatrixROW["resource_to"] == '') {
    				$rowKey = $clientOptimizationMatrixROW["resource_from"]." > ";
    			} else {
		...
		foreach ($fromDate as $fromDateKey => $fromDateValue) {
			...
			(SELECT
				...
				DATE_FORMAT(comp.date_created, '%m-%d-%Y') AS company_creation_date,
			...
			if (mysqli_num_rows($mainRESULT) > 0) {
				while ($mainROW = mysqli_fetch_array($mainRESULT)) {
					...
					$catsStatusDetailPopup = "personnel_id=".$mainROW["company_id"]."&personnel_name=".$mainROW["company_name"]."".$personnelManager."&personnel_type=".$personnelType."&start_date=".$startDate."&end_date=".$endDate;
					...
					if ($resourcesWorkingNumber > 0 || $employeeAverageMargin > 0) {
						foreach ($getAllResourceAndMargin as $getAllResourceAndMarginKey => $getAllResourceAndMarginItem) {
							if ((($resourcesWorkingNumber >= $getAllResourceAndMarginItem["resource_from"]) && ($resourcesWorkingNumber <= $getAllResourceAndMarginItem["resource_to"] || $getAllResourceAndMarginItem["resource_to"] == null)) && (($employeeAverageMargin >= $getAllResourceAndMarginItem["margin_min"]) && ($employeeAverageMargin <= $getAllResourceAndMarginItem["margin_max"] || $getAllResourceAndMarginItem["margin_max"] == null) )) {
								...
								$optimizationMatrixRank = $getAllResourceAndMarginItem["title"];
					...
					$mainTableData[] = array(
						...
						"company_creation_date" => $mainROW["company_creation_date"],
						...
						"cats_status_detail_popup" => $catsStatusDetailPopup,
						"total_unanswered_job_openings" => $mainROW["total_unanswered_job_openings"] != "" ? $mainROW["total_unanswered_job_openings"] : 0,
						"total_onhold_internal" => $mainROW["total_onhold_internal"] != "" ? $mainROW["total_onhold_internal"] : 0,
					...
					/*   Line Graph start */
					$optimizationMatrixColorMonth[$givenMonth][] = $optimizationMatrixColor;
					/*   Line Graph end */

					/*   Line Graph start */
					$optimizationMatrixLineGraph[$givenMonth][] = $optimizationMatrixColor;
					/*   Line Graph end */
					...
					$dipAndDullClient[$mainROW["company_id"]]['rank'][] = $optimizationMatrixRank;
					...
					if ($dipAndDullClientRank < $optimizationMatrixRank && $dipAndDullClientColor != $optimizationMatrixColor) {
						$dipAndDullClient[$mainROW["company_id"]]['clientType'] = "Dip";
					} elseif($dipAndDullClientRank < $optimizationMatrixRank && $dipAndDullClientColor == $optimizationMatrixColor) {
	...
	foreach ($optimizationMatrixColorMonth as $key => $value) {
	
		$counts = array_count_values($value);
		$countTheClientByColor[$key][] = array(
			"month" => $key,
			"count" => $counts['#83c48a'],
			"color" => 'Green',
			"colorCode" => '#83c48a'
	...
	/* for datapoint 1 start */
	foreach ($optimizationMatrixLineGraph as $key => $value) {
		$counts = array_count_values($value);
		$dateGiven = explode("/", $key);
		$dateModified = $dateGiven[0]."-".substr($dateGiven[1],-2);
		// $dateGiven = explode("/", $key);
		// $dateModified = date("F", strtotime($dateGiven[1]."-".$dateGiven[0]."-"."01"));
		$countTheClientByGraphForOne[$key][] = array(
			"y" => $counts['#dd7c7c'] != '' ? $counts['#dd7c7c'] : 0,
			"label" => $dateModified,
			"color" =>  "red"
		);
	}
	if (isset($_REQUEST["customized-multiple-month"])) { 
		foreach ($countTheClientByGraphForOne as $key => $valueGraph) {
			foreach ($valueGraph as $graphKey => $graphValue) {
				$dataPoints1[] = $graphValue;
			}
		}
	}
	/* for datapoint 1 end */

	/* for datapoint 2 start */
	foreach ($optimizationMatrixLineGraph as $key => $value) {
		$counts = array_count_values($value);
		$dateGiven = explode("/", $key);
		$dateModified = $dateGiven[0]."-".substr($dateGiven[1],-2);
		$countTheClientByGraphForTwo[$key][] = array(
			"y" => $counts['#e8ac43'] != '' ? $counts['#e8ac43'] : 0,
			"label" => $dateModified,
			"color" =>  "#e8ac43"
		);
	}
	if (isset($_REQUEST["customized-multiple-month"])) { 
		foreach ($countTheClientByGraphForTwo as $key => $valueGraph) {
			foreach ($valueGraph as $graphKey => $graphValue) {
				$dataPoints2[] = $graphValue;
			}
		}
	}
	/* for datapoint 2 end */

	/* for datapoint 3 start */
	foreach ($optimizationMatrixLineGraph as $key => $value) {
		$counts = array_count_values($value);
		$dateGiven = explode("/", $key);
		$dateModified = $dateGiven[0]."-".substr($dateGiven[1],-2);
		$countTheClientByGraphForThree[$key][] = array(
			"y" => $counts['#83c48a'] != '' ? $counts['#83c48a'] : 0,
			"label" => $dateModified,
			"color" =>  "green"
		);
	}
	if (isset($_REQUEST["customized-multiple-month"])) { 
		foreach ($countTheClientByGraphForThree as $key => $valueGraph) {
			foreach ($valueGraph as $graphKey => $graphValue) {
				$dataPoints3[] = $graphValue;
			}
		}
	}
	/* for datapoint 3 end */
...
<head>
	...
	<style>
		...
		.view-assigned-joborder-detail-popup .modal-lg,
		.view-unanswered-joborder-detail-popup .modal-lg {
			width: calc(100% - 100px);
		}
		.view-assigned-joborder-detail-popup .modal-header,
		...
		<div class="container-fluid">
			...
			<form action="index.php" method="post">
				<div class="row">
					<div class="col-md-4">
					<?php
						if (isset($_REQUEST["form-submit-button"]) && isset($_REQUEST["customized-multiple-month"])) {
					?>
						<table class="table table-striped table-bordered optimizationMatrixTable">
							<tr>
								<th colspan="5">Optimization Matrix</th>
							</tr>
							<?php
							...
							<tr>
								<th colspan="5">Average Margin (USD)</th>
							</tr>
						</table>

						<div class="scrollit">
						<div class="row main-section-row" style="margin-right: 0px;">
							<div class="col-md-12">
								<table class="table table-striped table-bordered">
								    <thead>
								    	<tr>
								    		<th>Color/Month</th>
								    	<?php 
								    		foreach ($countTheClientByColor as $key => $value) {
								    	?>
									    	<th><?php echo $key; ?></th>
								    	<?php
								  		}
								    ...
								    <tbody>
								     <?php 
								     	for ($i=0; $i < 3; $i++) { ?>
								     		<tr>
										      	<td style="background-color: <?php echo $value[$i]['colorCode']; ?> " ><?php echo $value[$i]['color']; ?></td>
										      	<?php 
										    		foreach ($countTheClientByColor as $key => $value) {
										    	?>

										        <td><?php 
										        if (empty($value[$i]['count'])) {
										        	echo "-";
										        } else {
										        	echo $value[$i]['count']; 
										        }
						...
						<div class="row main-section-row multiple-month-input">
							<div class="col-md-12">
								...
								<div class="input-group">
									...
									<input type="text" name="customized-multiple-month" class="form-control customized-multiple-month" value="<?php if (isset($_REQUEST['customized-multiple-month'])) { echo $_REQUEST['customized-multiple-month']; } ?>" placeholder="MM/YYYY" autocomplete="off">
					...
					<table class="table table-striped table-bordered customized-datatable">
						<thead>
							<tr class="thead-tr-style">
								...
								<th rowspan="3">Client Creation Date</th>
							...
							<?php } ?>
								<th colspan="15">Total No. of</th>
							<?php if ($sessionROW['user_type'] != 'Inside Post Sales') { ?>
						...
						<tbody>
							...
							<tr class="tbody-tr-style">
								...
								<td>
								<?php
									echo $mainTableDataValue["company_creation_date"];
								?>
								</td>
								<td>
								<?php
							...
							?>
								<td data-order="<?php echo $mainTableDataValue["total_canceled_job"]; ?>">
								<?php
									if ($mainTableDataValue["total_canceled_job"] != 0) {
								?>
									<a class="cancelled-onhold-joborder-popup hyper-link-text" data-popup="<?php echo $mainTableDataValue['cats_status_detail_popup']; ?>"
										data-status="Canceled" data-titlename="Canceled Joborder Details">
								<?php
								...
								<td data-order="<?php echo $mainTableDataValue["total_job"]; ?>">
								<?php
									if ($mainTableDataValue["total_job"] != 0) {
								?>
									<a class="assigned-joborder-detail-popup hyper-link-text" data-popup="<?php echo $mainTableDataValue['joborder_detail_popup']; ?>">
								<?php
								...
								<td data-order="<?php echo $mainTableDataValue["total_openings"]; ?>">
								<?php
									if ($mainTableDataValue["total_openings"] != 0) {
								?>
									<a class="assigned-joborder-detail-popup hyper-link-text" data-popup="<?php echo $mainTableDataValue['joborder_detail_popup']; ?>">
								<?php
								...
								<td data-order="<?php echo $mainTableDataValue["total_unanswered_job"]; ?>">
								<?php
									if ($mainTableDataValue["total_unanswered_job"] != 0) {
								?>
									<a class="unanswered-joborder-detail-popup hyper-link-text" data-popup="<?php echo $mainTableDataValue['joborder_detail_popup']; ?>">
								<?php
								...
								<td data-order="<?php echo $mainTableDataValue["total_unanswered_job_openings"]; ?>">
								<?php
									if ($mainTableDataValue["total_unanswered_job_openings"] != 0) {
								?>
									<a class="unanswered-joborder-detail-popup hyper-link-text" data-popup="<?php echo $mainTableDataValue['joborder_detail_popup']; ?>">
								<?php
								...
								<td data-order="<?php echo $mainTableDataValue["total_onhold_internal"]; ?>">
								<?php
									if ($mainTableDataValue["total_onhold_internal"] != 0) {
								?>
									<a class="cancelled-onhold-joborder-popup hyper-link-text" data-popup="<?php echo $mainTableDataValue['cats_status_detail_popup']; ?>"
										data-status="On Hold - Internal" data-titlename="OnHold-Internal Joborder Details">
								<?php
									echo $mainTableDataValue["total_onhold_internal"];
								?>
									</a>
								<?php
									} else {
								...
								<td data-order="<?php echo $mainTableDataValue["total_submission"]; ?>">
								<?php
									if ($mainTableDataValue["total_submission"] != 0) {
								?>
									<a class="cats-status-detail-popup hyper-link-text" data-popup="<?php echo $mainTableDataValue['cats_status_detail_popup']; ?>"
										data-status="400" data-titlename="Submission Details">
								<?php
								...
								<td data-order="<?php echo $mainTableDataValue["total_interview"]; ?>">
								<?php
									if ($mainTableDataValue["total_interview"] != 0) {
								?>
									<a class="cats-status-detail-popup hyper-link-text" data-popup="<?php echo $mainTableDataValue['cats_status_detail_popup']; ?>"
										data-status="500" data-titlename="Interview Details">
								<?php
								...
								<td data-order="<?php echo $mainTableDataValue["total_interview_decline"]; ?>">
								<?php
									if ($mainTableDataValue["total_interview_decline"] != 0) {
								?>
									<a class="cats-status-detail-popup hyper-link-text" data-popup="<?php echo $mainTableDataValue['cats_status_detail_popup']; ?>"
										data-status="560" data-titlename="Declined Interview Details">
								<?php
								...
								<td data-order="<?php echo $mainTableDataValue["total_offer"]; ?>">
								<?php
									if ($mainTableDataValue["total_offer"] != 0) {
								?>
									<a class="cats-status-detail-popup hyper-link-text" data-popup="<?php echo $mainTableDataValue['cats_status_detail_popup']; ?>"
										data-status="600" data-titlename="Offered Details">
								<?php
								...
								<td data-order="<?php echo $mainTableDataValue["total_placed"]; ?>">
								<?php
									if ($mainTableDataValue["total_placed"] != 0) {
								?>
									<a class="cats-status-detail-popup hyper-link-text" data-popup="<?php echo $mainTableDataValue['cats_status_detail_popup']; ?>"
										data-status="800" data-titlename="Placed Details">
								<?php
								...
								<td data-order="<?php echo $mainTableDataValue["total_extension"]; ?>">
								<?php
									if ($mainTableDataValue["total_extension"] != 0) {
								?>
									<a class="cats-status-detail-popup hyper-link-text" data-popup="<?php echo $mainTableDataValue['cats_status_detail_popup']; ?>"
										data-status="620" data-titlename="Extension Details">
								<?php
								...
								<td data-order="<?php echo $mainTableDataValue["total_failed_delivery"]; ?>">
								<?php
									if ($mainTableDataValue["total_failed_delivery"] != 0) {
								?>
									<a class="cats-status-detail-popup hyper-link-text" data-popup="<?php echo $mainTableDataValue['cats_status_detail_popup']; ?>"
										data-status="900" data-titlename="Delivery Failed Details">
								<?php
								...
								<td data-order="<?php echo $mainTableDataValue["total_termination"]; ?>">
								<?php
									if ($mainTableDataValue["total_termination"] != 0) {
								?>
									<a class="termination-detail-popup hyper-link-text" data-popup="<?php echo $mainTableDataValue['cats_status_detail_popup']; ?>">
								<?php
								...
								<td data-order="<?php echo $mainTableDataValue["resources_working"]; ?>">
								<?php
									if (($user == "1" || $user == "3") && $sessionROW['user_type'] != 'Inside Post Sales') {
								?>
							...
							<?php if ($sessionROW['user_type'] != 'Inside Post Sales') { ?>
								<td data-order="<?php echo $mainTableDataValue["average_actual_margin"]; ?>">
								<?php
									echo $mainTableDataValue["average_actual_margin"];
								?>
								</td>
								<td data-order="<?php echo $mainTableDataValue["final_actual_margin"]; ?>">
								<?php
									if ($user == "1" || $user == "3") {
								?>
									<a class="margin-detail-popup" data-popup="<?php echo $mainTableDataValue['margin_detail_popup']; ?>" data-titletype="Actual">
								...
								<td data-order="<?php echo $mainTableDataValue["optimization_matrix_rank"]["optimization_matrix_title"]; ?>" style="color: #000;background-color: <?php echo $mainTableDataValue["optimization_matrix_rank"]["optimization_matrix_color"]; ?>">
								<?php
						...
						<tfoot>
							...
							<?php } ?>
								...
								<th><?php echo array_sum(array_column($mainTableData, "total_onhold_internal")); ?></th>
...
<script>
	...
	$(document).on("click", ".cancelled-onhold-joborder-popup", function(e){
		e.preventDefault();
		$("#divLoading").addClass("show");
		let titleName = $(this).data("titlename");
		$.ajax({
			type: "POST",
			url: "<?php echo DIR_PATH; ?>functions/cancelled-onhold-joborder-popup.php",
			data: "status="+$(this).data("status")+"&"+$(this).data("popup"),
			success: function(response) {
				$("#divLoading").removeClass("show");
				$(".view-cancelled-onhold-joborder-popup").modal("show");
				$(".cancelled-onhold-table-section, .cancelled-onhold-joborder-title").html("");
				$(".cancelled-onhold-table-section").html(response);
				$(".cancelled-onhold-joborder-title").html(titleName);
				$(".scrollable-datatable").DataTable({
					"lengthMenu": [[10, 25, 50, -1], [10, 25, 50, "All"]],
				    dom: "Blfrtip",
				    "aaSorting": [[0,"asc"]],
	...
	$(document).on("click", ".termination-detail-popup", function(e){
		e.preventDefault();
		$("#divLoading").addClass("show");

		$.ajax({
			type: "POST",
			url: "<?php echo DIR_PATH; ?>functions/termination-detail-popup.php",
			data: $(this).data("popup"),
			success: function(response) {
				$("#divLoading").removeClass("show");
				$(".view-termination-detail-popup").modal("show");
				$(".termination-table-section").html("");
				$(".termination-table-section").html(response);
				$(".scrollable-datatable").DataTable({
					"lengthMenu": [[10, 25, 50, -1], [10, 25, 50, "All"]],
				    dom: "Blfrtip",
				    "aaSorting": [[0,"asc"]],
	...
	$(document).on("click", ".unanswered-joborder-detail-popup", function(e){
		e.preventDefault();
		$("#divLoading").addClass("show");
		$.ajax({
			type: "POST",
	...
	$(document).on("click", ".assigned-joborder-detail-popup", function(e){
		e.preventDefault();
		$("#divLoading").addClass("show");
		$.ajax({
			type: "POST",
			url: "<?php echo DIR_PATH; ?>functions/assigned-joborder-detail-popup.php",
			data: $(this).data("popup"),
			success: function(response) {
				$("#divLoading").removeClass("show");
				$(".view-assigned-joborder-detail-popup").modal("show");
				$(".margin-table-section").html("");
				$(".margin-table-section").html(response);
				$(".scrollable-datatable").DataTable({
					"lengthMenu": [[10, 25, 50, -1], [10, 25, 50, "All"]],
	...
	$(document).on("click", ".cats-status-detail-popup", function(e){
		e.preventDefault();
		$("#divLoading").addClass("show");
		let titleName = $(this).data("titlename");
		$.ajax({
			type: "POST",
			url: "<?php echo DIR_PATH; ?>functions/cats-status-detail-popup.php",
			data: "status="+$(this).data("status")+"&"+$(this).data("popup"),
			success: function(response) {
				$("#divLoading").removeClass("show");
				$(".view-cats-status-detail-popup").modal("show");
				$(".cats-status-table-section, .cats-status-title").html("");
				$(".cats-status-table-section").html(response);
				$(".cats-status-title").html(titleName);
				$(".scrollable-datatable").DataTable({
					"lengthMenu": [[10, 25, 50, -1], [10, 25, 50, "All"]],
				    dom: "Blfrtip",
				    "aaSorting": [[0,"asc"]],
