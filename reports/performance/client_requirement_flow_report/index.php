...
<html>
<head>
	<title>Client Requirement Flow Report</title>
...
<body>
	...
	<section>
		<div class="container-fluid">
			<div class="row">
				<div class="col-md-12 report-title">Client Requirement Flow Report</div>
					...
					<table class="table table-striped table-bordered customized-datatable">
						<thead>
							...
							<?php } ?>
								...
								<th>Percentage(%) of Total Openings</th>
								<th>Average Bill Rate</th>
							</tr>
						</thead>
						<tbody>
							<?php
								...
								foreach ($fromDate as $fromDateKey => $fromDateValue) {
									...
									$mainQUERY = "SELECT
										...
										e.average_bill,
										(SELECT
											SUM(job.openings) AS final_openings
										FROM
											company AS comp
										...
										AND
											date_format(job.date_created, '%Y-%m-%d') BETWEEN '$startDate' AND '$endDate') AS final_openings
									FROM
									(SELECT
										comp.company_id,
									...
									WHERE
										(e.total_openings != '' OR e.total_salary != '' OR e.average_bill != '')";
							...
							<?php } ?>
								...
								<td><?php echo round(($mainROW["total_openings"] * 100) / $mainROW["final_openings"], 2)."%"; ?></td>
								<td>
								<?php
									if ($mainROW["average_bill"] != "") {
										echo $mainROW["average_bill"];
									} else {
										echo "0";
									}
								?>
