<?php
	...
	$templateList = array(
		"sales_interview_questions" => "Interview Questions",
		"sales_client_qualification_checklist" => "Client Qualification Questions",
		"sales_premeeting_questions" => "Pre-Meeting Questions"
	);
	...
	$query = "SELECT
		count(DISTINCT candidate_id) as total_count,
		screening_type";
		...
		foreach ($valueList as $key => $value) {
			if ($key == 'template') {
				...
				if ($value == 0){
					...
					$result .='<td align="center"><a style="cursor: pointer;" class="sales-screening-popup hyper-link-text" data-popup="'.$firstDateForPopup.'/'.$lastDateForPopup.'" data-status="'.$valueList['template'].'" data-titlename="'.$templateList[$valueList['template']].' ('.$displayDateList[$key].')" >';
					$result .= $value;
					$result .='</a></td>';
