<?php
	...
	$templateList = array(
		"VectorVMS" => "VectorVMS",
		"GenesisHealthcare" => "GenesisHealthcare", 
		"AmeriHealth_VT" => "AmeriHealth", 
		"Medefis" => "Medefis", 
		"Total-MSP" => "Total-MSP",
		"Vizient" => "Vizient",
		"Hallmark_Healthcare" => "Hallmark_Healthcare",
		"Workforce_Logiq" => "Workforce_Logiq"
	);
	...
	$query = "SELECT `job_source`, COUNT(id) as jobs_count ";
	...
	$query .= " FROM vtech_mappingdb.ats_mapped_jobs";
	...
	if ($multipleMonth !== '') {
		...
		$query .= " WHERE CONCAT(YEAR(created_date), 'Q', QUARTER(created_date)) in ('".$multipleQuarter."')
					GROUP BY job_source,year, quarter
		...
		$query .= " WHERE created_date BETWEEN '".$fromDate[2]."-".$fromDate[0]."-".$fromDate[1]." 00:00:00' and '".$toDate[2]."-".$toDate[0]."-".$toDate[1]." 23:59:59' GROUP BY job_source";
	}
	...
	if ($rowcount > 0) {
		...
		while($resultRow = mysqli_fetch_assoc($resultQuery)) {
			foreach ($templateUsage as $key => $value) {
				if ($value["Source Portal"] == $resultRow["job_source"]) {
					if (isset($resultRow["datec"]) && isset($value[$resultRow["datec"]]) && ($isMultipleMonth || $isMultipleQuarter)) {
						$templateUsage[$key][$resultRow["datec"]] = $resultRow["jobs_count"];
					} else {
						$templateUsage[$key]["date_range"] = $resultRow["jobs_count"];
					}
				}
			}
		}
			...
			foreach ($valueList as $key => $value) {
				if ($key == 'Source Portal') {
					...
					if ($value == 0){
						...
						$result .='<td align="center"><a style="cursor: pointer;" class="auto-import-jobs-popup hyper-link-text" data-popup="'.$firstDateForPopup.'/'.$lastDateForPopup.'" data-status="'.$valueList['Source Portal'].'" data-titlename="'.$templateList[$valueList['Source Portal']].' ('.$displayDateList[$key].')" >';
						$result .= $value;
						$result .='</a></td>';
