<?php
	...
	$searchQuery = "SELECT
		count(DISTINCT cjsh.candidate_id) AS total_count,
		...
		foreach ($valueList as $key => $value) {
			if ($key == 'Status') {
				...
				if ($value == 0){
					...
					$result .='<td align="center"><a style="cursor: pointer;" class="advance-search-detail-popup hyper-link-text" data-popup="'.$firstDateForPopup.'/'.$lastDateForPopup.'" data-status="'.$valueList['Status'].'" data-titlename="'.ucwords(implode(" ", explode("_", $valueList['Status']))).' ('.$displayDateList[$key].')" >';
					$result .= $value;
					$result .='</a></td>';
