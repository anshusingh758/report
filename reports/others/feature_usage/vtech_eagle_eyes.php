<?php
	...
	$query = "SELECT `type`, count(DISTINCT ltsvl.user_id) AS userId";
	...
	$query .= " FROM vtech_mappingdb.live_tracker_status_view_log AS ltsvl";
			...
			foreach ($valueList as $key => $value) {
				if ($key == 'Type') {
					...
					if ($value == 0){
						...
						$result .='<td align="center"><a style="cursor: pointer;" class="vtech-eagle-eyes-popup hyper-link-text" data-popup="'.$firstDateForPopup.'/'.$lastDateForPopup.'" data-status="'.$valueList['Type'].'" data-titlename="'.$templateList[$valueList['Type']].' ('.$displayDateList[$key].')" >';
						$result .= $value;
						$result .='</a></td>';
