...
<html>
<head>
	<title>US BD Matrix Report</title>
...
<body>
	...
	<section>
		<div class="container-fluid">
			<div class="row">
				<div class="col-md-12 report-title">US BD Matrix Report</div>
				...
				<div class="row main-section-row">
					<div class="col-md-4 col-md-offset-4">
						...
						<select id="personnel-list" class="<?php if (isset($_REQUEST["personnel-list"])) { echo "customized-selectbox-without-all"; } else { echo "customized-selectbox-with-all"; } ?>" name="personnel-list[]" multiple required>
							<?php
								$isSelected = "";
								$salesGroupPersonnel = salesGroupPersonnelList($sales_connect,"3");
			...
			<div class="row report-bottom-style">
				<div class="col-md-12">
					<table class="table table-striped table-bordered customized-datatable">
						...
						<tbody>
							<?php
								$totalAccounts = $totalContacts = $totalFirstTouch = $totalMeaningfulData = $totalMeetings = $totalShowUps = $totalFollowUps = $totalPipeline = $totalWorking = $totalSubmitted = $totalSecondStage = $totalWon = $totalRequirements = $totalGrossProfit = array();
								...
								foreach ($fromDate as $fromDateKey => $fromDateValue) {
									...
									if (mysqli_num_rows($mainRESULT) > 0) {
										while ($mainROW = mysqli_fetch_array($mainRESULT)) {
											...
											$dataPopUp = "type=onsite&person=".$personnelNameValue."&startDate=".$startDate."&endDate=".$endDate;
							?>
