...
<html>
<head>
	<title>BD Activities By Personnel Report</title>
...
<body>
	...
	<section>
		<div class="container-fluid">
			<div class="row">
				<div class="col-md-12 report-title">BD Activities By Personnel Report</div>
...
<?php
		...
		$bdgPersonnelGroup = salesGroupPersonnelList($sales_connect,"1");
?>
					...
					<table class="table table-striped table-bordered customized-datatable">
						<thead>
							...
							<?php } ?>
								...
								<th colspan="8">Total No. of Logs in</th>
						...
						<tbody>
							<?php
								$totalAccounts = $totalContacts = $totalCall = $totalEmail = $totalComment = $totalMeaningfulData = $totalMeetings = $totalShowUps = $totalFirstTouch = $totalFollowUps = $totalPipeline = $totalWorking = $totalSubmitted = $totalSecondStage = $totalWon = $totalLoss = $totalRequirements = array();
								...
								foreach ($fromDate as $fromDateKey => $fromDateValue) {
									...
									(SELECT
									    ...
									    SUM(IF(((los_main.loss_main IS NULL) AND (los_log.loss_log IS NOT NULL)), 1, IF(((los_main.loss_main IS NOT NULL) AND (los_log.loss_log IS NULL)), 1, IF(((los_main.loss_main IS NOT NULL) AND (los_log.loss_log IS NOT NULL)), 1, 0)))) AS total_loss
									FROM
									(SELECT
									...
									AND
									    DATE_FORMAT(FROM_UNIXTIME(opp.createDate), '%Y-%m-%d') BETWEEN '$startDate' AND '$endDate'
									GROUP BY opportunity_id,personnel_name) AS los_main ON los_main.opportunity_id = all_main.opportunity_id AND los_main.personnel_name = all_main.personnel_name
									LEFT JOIN
									(SELECT
									    opp.id AS opportunity_id,
									    CONCAT(u.firstName,' ',u.lastName) AS personnel_name,
									...
									AND
									    DATE_FORMAT(FROM_UNIXTIME(chg.timestamp), '%Y-%m-%d') BETWEEN '$startDate' AND '$endDate'
									GROUP BY opportunity_id,personnel_name) AS los_log ON los_log.opportunity_id = all_main.opportunity_id AND los_log.personnel_name = all_main.personnel_name
									WHERE
									    (pip_main.pipeline_main != '' OR wor_main.working_main != '' OR sub_main.submitted_main != '' OR sec_main.second_stage_main != '' OR won_mainn.won_main != '' OR los_main.loss_main != '' OR pip_log.pipeline_log != '' OR wor_log.working_log != '' OR sub_log.submitted_log != '' OR sec_log.second_stage_log != '' OR won_logg.won_log != '' OR los_log.loss_log != '')
									GROUP BY personnel_name) AS h ON h.personnel_name = a.personnel_name
									WHERE
										(b.total_accounts != '' OR b.total_contacts != '' OR c.total_first_touch != '' OR d.total_follow_ups != '' OR e.total_show_ups != '' OR f.total_requirements != '' OR g.total_call != '' OR g.total_email != '' OR g.total_comment != '' OR g.total_meeting != '' OR g.total_meaningful != '' OR h.total_pipeline != '' OR h.total_working != '' OR h.total_submitted != '' OR h.total_second_stage != '' OR h.total_won != '' OR h.total_loss != '' OR a.personnel_status != '0')
							...
							<?php } ?>
								...
								<?php
									if ($mainROW["total_first_touch"] == "") {
										echo $totalFirstTouch[] = "0";
									} else {
										echo $totalFirstTouch[] = $mainROW["total_first_touch"];
									}
								?>
								</td>
								<td>
								<?php
									if ($mainROW["total_follow_ups"] == "") {
										echo $totalFollowUps[] = "0";
									} else {
										echo $totalFollowUps[] = $mainROW["total_follow_ups"];
									}
								?>
								</td>
								<td>
								...
								<?php
									if (in_array($mainROW["personnel_name"], $bdgPersonnelGroup)) {
								...
								<?php
										...
										if ($mainROW["total_won"] == "") {
											...
											echo $totalWon[] = $mainROW["total_won"];
										}
									}
								?>
								</td>
								<td>
								<?php
									if ($mainROW["total_loss"] == "") {
										echo $totalLoss[] = "0";
									} else {
										echo $totalLoss[] = $mainROW["total_loss"];
									}
								?>
								</td>
								<td>
						...
						<tfoot>
							...
							<?php } ?>
								...
								<th><?php echo array_sum($totalLoss); ?></th>
