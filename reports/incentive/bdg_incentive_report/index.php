...
<html>
<head>
	<title>BDG Incentive Report</title>
...
<body>
	...
	<section>
		<div class="container-fluid">
			<div class="row">
				<div class="col-md-12 text-center" style="font-size: 30px;background-color: #aaa;color: #111;padding: 10px;">BDG Incentive Report</div>
							...
							<div class="col-md-6 col-md-offset-6">
								...
								<select class="form-control" id="personnel_name" name="personnel_name[]" style="border: 1px solid #aaa;border-radius: 0px;" multiple>
									<?php
										...
										AND
											concat(u.firstName,' ',u.lastName) != 'Mohsin Shaikh'
										AND
											concat(u.firstName,' ',u.lastName) != 'Simrat Singh'");
						...
						<table id="PIRdata" class="table table-striped table-bordered">
							...
							<tbody>
									...
									foreach($personneldata AS $personneldataX){
										...
										foreach($enameX AS $key => $enameX2){
											...
											if($newAccCrack[$key]=='0'){
												/////////Incentive Amount//////////////////
												$iamountQ="SELECT * FROM incentive_criteria WHERE personnel='Proposal' AND comment=''";
										...
										GROUP BY cxopp.id");
										$contract_name=$sign_date=$contract_type=$sowAMT=$mspAMT=$directAMT=$productsaleAMT=array();
										while($contractROW=mysqli_fetch_array($contractQRY)){
											...
											$contractTypeQRY=mysqli_query($misReportsConn,"SELECT value FROM incentive_criteria WHERE personnel='Proposal' AND comment='$contract_type_value'");
											if(mysqli_num_rows($contractTypeQRY)>0){
												$contractTypeROW=mysqli_fetch_array($contractTypeQRY);
												if($contractROW['c_contract_type']=='SOW'){
													$sowAMT[]=$contractTypeROW['value'];
													$mspAMT[]='0';
													$directAMT[]='0';
													$productsaleAMT[]='0';
												}elseif($contractROW['c_contract_type']=='MSP Staffing' OR $contractROW['c_contract_type']=='MSP Staffing'){
													$sowAMT[]='0';
													$mspAMT[]=$contractTypeROW['value'];
													$directAMT[]='0';
													$productsaleAMT[]='0';
												}elseif($contractROW['c_contract_type']=='Direct Staffing' OR $contractROW['c_contract_type']=='Direct Staffing'){
													$sowAMT[]='0';
													$mspAMT[]='0';
										...
										$selectQRY="SELECT * FROM incentive_data WHERE person_name = '$personneldataX' AND period = '$dtX' AND type = 'Proposal'";
								...
								<tr style="background-color: #c3dcf4;">
									...
									<td style="text-align: center;vertical-align: middle;"><?php echo $selectROW['bd_sow']; ?></td>
								...
								<tr>
									...
									<input type="hidden" id="mainamount<?php echo $iii; ?>" value="<?php echo array_sum($totINC)+array_sum($sowAMT)+array_sum($mspAMT)+array_sum($directAMT)+array_sum($productsaleAMT); ?>">

									<input type="hidden" name="person_name[<?php echo $iii; ?>]" value="<?php echo ucwords($personneldataX); ?>">
									<input type="hidden" name="type_data[<?php echo $iii; ?>]" value="<?php echo "Proposal"; ?>">
									...
									<input type="hidden" name="bd_sow[<?php echo $iii; ?>]" value="<?php echo array_sum($sowAMT); ?>">
									...
									<input type="hidden" name="final_incentive[<?php echo $iii; ?>]" value="<?php echo array_sum($totINC)+array_sum($sowAMT)+array_sum($mspAMT)+array_sum($directAMT)+array_sum($productsaleAMT); ?>">
									<input type="hidden" name="detail_link[<?php echo $iii; ?>]" value="<?php echo LOCAL_REPORT_PATH; ?>/incentive/bdg_incentive_report/index.php?multimonth=<?php echo urlencode($dtX); ?>&personnel_name=<?php echo urlencode($personneldataX); ?>&PIRsubmit2=&response_type=1">
									...
									<td style="text-align: center;vertical-align: middle;"><?php echo array_sum($sowAMT); ?></td>
									...
									<td style="text-align: center;vertical-align: middle;font-weight: bold;"><?php echo array_sum($totINC)+array_sum($sowAMT)+array_sum($mspAMT)+array_sum($directAMT)+array_sum($productsaleAMT); ?></td>
									<td style="text-align: center;vertical-align: middle;">
					...
					<table id="PIRdata2" class="table table-striped table-bordered">
						<thead>
							<tr style="background-color: #bbb;color: #000;font-size: 13px;">
								...
								<th style="text-align: center;vertical-align: middle;" colspan="4">Signing Amount</th>
							</tr>
							<tr style="background-color: #bbb;color: #000;font-size: 13px;">
								<th style="text-align: center;vertical-align: middle;">SOW</th>
								<th style="text-align: center;vertical-align: middle;">MSP</th>
						...
						<tbody>
							<?php
								...
								AND
									cxopp.createDate BETWEEN '$sdateX2' AND '$tdateX2'
								GROUP BY cxopp.id");
								$contract_name=$accountName=$contractNo=$sign_date=$contract_type=$sowAMT=$mspAMT=$directAMT=$productsaleAMT=array();
								while($contractROW=mysqli_fetch_array($contractQRY)){
									$contract_name[]=$contractROW['name'];
									...
									if(mysqli_num_rows($contractTypeQRY)>0){
										...
										if($contractROW['c_contract_type']=='SOW'){
											...
											$directAMT[]='0';
											$productsaleAMT[]='0';
										}elseif($contractROW['c_contract_type']=='MSP Staffing' OR $contractROW['c_contract_type']=='MSP Staffing + SOW'){
											$sowAMT[]='0';
											$mspAMT[]=$contractTypeROW['value'];
											$directAMT[]='0';
											$productsaleAMT[]='0';
							...
							<tr>
								...
								<td style="text-align: center;vertical-align: middle;"><?php echo $sowAMT[$key]; ?></td>
							...
							<?php
										...
										$responseArray['opportunityList'][] = array('opportunity' => $contract_name[$key],
											'account_name' => $accountName[$key],
											'contract_no' => $contractNo[$key],
											'contract_type' => $contract_type[$key],
											'sign_date' => $sign_date[$key],
											'sow_ammount' => $sowAMT[$key],
						...
						<tfoot>
							<tr style="background-color: #bbb;color: #000;">
								...
								<th style="text-align: center;vertical-align: middle;"><?php echo array_sum($sowAMT); ?></th>
							...
							<tr style="background-color: #bbb;color: #000;">
								...
								<th style="text-align: center;vertical-align: middle;" colspan="4"><?php echo $totBONUS=array_sum($sowAMT)+array_sum($mspAMT)+array_sum($directAMT)+array_sum($productsaleAMT); ?></th>
							</tr>
						</tfoot>
					</table>
				</div>
