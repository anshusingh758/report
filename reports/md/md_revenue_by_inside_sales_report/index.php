...
<html>
<head>
	<title>MD Revenue by Inside Sales Report</title>
	...
	<style>
		...
		.view-working-resources-detail-by-md-report .modal-lg {
...
<body>
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					<ul style="font-weight: bold;">
						<li>This Report is useful for viewing MD Revenue detail of inside sales personnel for selected daterange.
							<ul>
	...
	<section>
		<div class="container-fluid">
			<div class="row">
				<div class="col-md-12 report-title">MD Revenue by Inside Sales Report</div>
				...
				<div class="row main-section-row">
					<div class="col-md-4 col-md-offset-4">
						...
						<select id="personnel-list" class="<?php if (isset($_REQUEST["personnel-list"])) { echo "customized-selectbox-without-all"; } else { echo "customized-selectbox-with-all"; } ?>" name="personnel-list[]" multiple required>
							<?php
								$isSelected = "";
								$personnelList = catsExtraFieldPersonnelList($catsConn,"Inside Sales");
								sort($personnelList);
...
<?php
		...
		foreach ($fromDate as $fromDateKey => $fromDateValue) {
			...
			foreach ($personnelData as $personnelDataKey => $personnelDataValue) {
				...
				WHERE
					(si.c_inside_sales1 = '$personnelDataValue' OR si.c_inside_sales2 = '$personnelDataValue' OR si.c_research_by = '$personnelDataValue')
				AND
				...
				if (mysqli_num_rows($mainRESULT) > 0) {
					while ($mainROW = mysqli_fetch_array($mainRESULT)) {
						...
						$sharedWith = array_unique(explode(",", $mainROW["shared_with"]));
						...
						//if ($totalHour > "0") {
							...
							$sharedHours = round(($totalHour * $empShare), 2);

							$sharedRevenue = round(($totalRevenue * $empShare), 2);

							$sharedPay = round(($totalPay * $empShare), 2);

							$sharedMargin = round(($totalGP * $empShare), 2);
							...
							$finalReportArray[$personnelDataValue][$givenMonth][] = array(
								...
								"total_hours" => $sharedHours,
								"total_revenue" => $sharedRevenue,
								"total_pay" => $sharedPay,
								"total_gp" => $sharedMargin 
							);
						//}
					}
				}
				...
				<div class="col-md-12">
					<table class="table table-striped table-bordered customized-datatable">
						...
						<tbody>
								...
								foreach ($finalReportArray as $finalReportKey => $finalReportValue) {
									...
									foreach ($finalReportValue as $finalReportValueKey => $finalReportMonth) {
										...
										foreach ($finalReportMonth as $finalReportMonthKey => $finalReportMonthItem) {
											$resourceArray[] = $finalReportMonthItem["employee_share"];
											...
											$dataResources[$finalReportKey][] = array(
												...
												"total_share" => $finalReportMonthItem["total_share"],
												"shared_with" => $finalReportMonthItem["shared_with"],
										...
										foreach ($finalCountArray[$finalReportValueKey] as $finalCountArrayKey => $finalCountArrayValue) {
											$resourcesPerc = round(((array_sum($resourceArray) * 100) / $finalCountArrayValue["resources"]), 2);
							...
							<?php } ?>
								<td><a class="resources-detail-popup" data-popup='<?php echo json_encode($dataResources, true); ?>'><?php echo $resourceArraySum[] = array_sum($resourceArray); ?></a></td>
...
<script>
	...
	$(document).on("click", ".resources-detail-popup", function(e){
		...
		$.ajax({
			type: "POST",
			url: "<?php echo DIR_PATH; ?>functions/working-resources-detail-popup-by-md-sales.php",
