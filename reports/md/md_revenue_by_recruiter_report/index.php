...
<html>
<head>
	<title>MD Revenue by Recruiter Report</title>
...
<body>
	...
	<section>
		<div class="container-fluid">
			<div class="row">
				<div class="col-md-12 report-title">MD Revenue by Recruiter Report</div>
			...
			<form action="index.php" method="post">
				...
				<div class="row main-section-row">
					<div class="col-md-4 col-md-offset-4">
						...
						<select id="recruiter-list" class="<?php if (isset($_REQUEST["recruiter-list"])) { echo "customized-selectbox-without-all"; } else { echo "customized-selectbox-with-all"; } ?>" name="recruiter-list[]" multiple required>
							<?php
								$recruiterList = catsUserList($catsConn);
								foreach ($recruiterList as $recruiterKey => $recruiterValue) {
									$isSelected = "";
									if (in_array($recruiterValue['id'], $_REQUEST["recruiter-list"])) {
										$isSelected = " selected";
									}
									echo "<option value='".$recruiterValue['id']."'".$isSelected.">".$recruiterValue['name']."</option>";
								}
							?>
						</select>
					</div>
		...
		foreach ($fromDate as $fromDateKey => $fromDateValue) {
			...
			if (mysqli_num_rows($mainRESULT) > 0) {
				while ($mainROW = mysqli_fetch_array($mainRESULT)) {
					...
					$finalReportArray[$mainROW["recruiter_id"]][$givenMonth][] = array(
						"employee_id" => $mainROW["employee_id"],
						"recruiter_name" => $mainROW["recruiter_name"],
						"recruiter_manager" => $mainROW["recruiter_manager"],
					...
					<table class="table table-striped table-bordered customized-datatable">
						...
						<tbody>
							<?php
								foreach ($finalReportArray as $finalReportKey => $finalReportValue) {
									...
									foreach ($finalReportValue as $finalReportValueKey => $finalReportMonth) {
										...
										$recruiterName = $recruiterManager = "";
										...
										foreach ($finalReportMonth as $finalReportMonthKey => $finalReportMonthItem) {
											...
											$recruiterName = $finalReportMonthItem["recruiter_name"];
											$recruiterManager = $finalReportMonthItem["recruiter_manager"];
										}
							...
							<tr class="tbody-tr-style">
								<td><?php echo ucwords($recruiterName); ?></td>
								<td><?php echo ucwords($recruiterManager); ?></td>
							<?php if (isset($_REQUEST["customized-multiple-month"])) { ?>
