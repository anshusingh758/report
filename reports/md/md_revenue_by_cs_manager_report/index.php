...
<html>
<head>
	<title>MD Revenue by CS Manager Report</title>
...
<body>
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					<ul style="font-weight: bold;">
						<li>This Report is useful for viewing MD Revenue detail of CS Manager for selected daterange.
							<ul>
								<li>Here, You can view each CS manager's performance by client on the basis of <span style="color: red;">history changes</span> between given daterange.</li>
	...
	<section>
		<div class="container-fluid">
			<div class="row">
				<div class="col-md-12 report-title">MD Revenue by CS Manager Report</div>
...
<?php
		...
		foreach ($fromDate as $fromDateKey => $fromDateValue) {
			...
			$personnelArray = $resources = $hours = $revenue = $pay = $gp = array();
			...
			FROM
			    ...
			    LEFT JOIN cats.user AS u ON u.user_id = si.c_company_manager_id
			    LEFT JOIN vtechhrm.employeetimeentry AS ete ON ete.employee = e.id
			WHERE
				CONCAT(u.first_name,' ',u.last_name) IN ($personnelData)
			AND
			...
			if (mysqli_num_rows($mainRESULT) > 0) {
				while ($mainROW = mysqli_fetch_array($mainRESULT)) {
					...
					if ($totalHour > "0") {
						...
						$finalReportArray[$mainROW["personnel"]][$givenMonth][] = array(
