...
<html>
<head>
	<title>MD Revenue by Industry Report</title>
...
<body>
	...
	<section>
		<div class="container-fluid">
			<div class="row">
				<div class="col-md-12 report-title">MD Revenue by Industry Report</div>
			...
			<form action="index.php" method="post">
				...
				<div class="row main-section-row">
					<div class="col-md-4 col-md-offset-4">
						...
						<select id="industries-list" class="customized-selectbox-with-all" name="industries-list[]" multiple required>
							<?php
								$industriesList = catsIndustriesList($catsConn);
								foreach ($industriesList as $industriesKey => $industriesValue) {
									echo "<option value='".$industriesValue."'>".$industriesValue."</option>";
								}
							?>
						</select>
					</div>
...
<?php
		...
		$industriesData = "'".implode("', '",$_REQUEST['industries-list'])."'";
		...
		foreach ($fromDate as $fromDateKey => $fromDateValue) {
			...
			FROM
				...
				LEFT JOIN cats.extra_field AS ef ON ef.data_item_id = comp.company_id AND ef.field_name = 'Industry Type'
			...
			if (mysqli_num_rows($mainRESULT) > 0) {
				while ($mainROW = mysqli_fetch_array($mainRESULT)) {
					...
					$finalReportArray[$mainROW["industries"]][$givenMonth][] = array(
