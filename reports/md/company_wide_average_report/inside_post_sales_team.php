...
?>
	<script>
		$(document).ready(function(){
			var insidePostSalesTeamReport = $(".inside-post-sales-team-report").DataTable({
			    dom: "Bfrtip",
			    "bPaginate": false,
			    "bFilter": false,
			    bInfo: false,
	...
	if ($_POST) {
		...
		foreach ($dateRange as $dateRangeKey => $dateRangeValue) {
			...
			while ($joinedUserROW = mysqli_fetch_array($joinedUserQUERY)) {
			    ...
			    $joiningDate = $joinedUserROW["date_of_joining"];
			    ...
			    FROM
			        ...
			        LEFT JOIN cats.candidate_joborder_status_history AS sub ON sub.joborder_id = j.joborder_id AND sub.status_to = '400' AND DATE_FORMAT(sub.date, '%Y-%m-%d') > '$joiningDate'
			        LEFT JOIN cats.candidate_joborder_status_history AS inter ON inter.joborder_id = j.joborder_id AND inter.status_to = '500' AND DATE_FORMAT(inter.date, '%Y-%m-%d') > '$joiningDate'
			        LEFT JOIN cats.candidate_joborder_status_history AS place ON place.joborder_id = j.joborder_id AND place.status_to = '800' AND DATE_FORMAT(place.date, '%Y-%m-%d') > '$joiningDate'
			        LEFT JOIN vtech_mappingdb.system_integration AS si ON si.c_inside_post_sales = ef.value
		...
		if (array_sum(array_column($totalReport, "total_personnel")) == 0) {
			...
			$output = '<div class="row">
				...
				<div class="col-md-12">
					<table class="table table-striped table-bordered inside-post-sales-team-report">
						<thead>
