...
<?php
	if ($_POST) {
		...
		foreach ($dateRange as $dateRangeKey => $dateRangeValue) {
			...
			$taxRate = $mspFees = $primeCharges = $candidateRate = $totalHour = "";
			...
			$mainQUERY = "SELECT
				COUNT(DISTINCT a.recruiter_id) AS total_personnel,
				SUM(a.recruiter_live_status) AS total_active_personnel,
			...
			(SELECT
				...
				IF(u.access_level != 0, 1, 0) AS recruiter_live_status
			FROM
				user AS u
			...
			if (mysqli_num_rows($mainRESULT) > 0) {
				while ($mainROW = mysqli_fetch_array($mainRESULT)) {
					...
					$averageReport[] = array(
						...
						"total_unanswered_job" => round((($mainROW["total_unanswered_job"] / $mainROW["total_personnel"]) / $totalDays), 2),
					...
					<table class="table table-striped table-bordered cs-team-report">
						<thead>
							...
							if ($dateRangeType == "month") {
								...
								$output .= '<th colspan="13">Average Daily</th>
						...
						foreach ($averageDailyReport as $averageDailyReportKey => $averageDailyReportValue) {
							$output .= '<tr>';
								...
								$output .= '<td>'.$averageDailyReportValue["total_job"].'</td>
					...
					<table class="table table-striped table-bordered cs-team-report">
						<thead>
							...
							if ($dateRangeType == "month") {
								...
								$output .= '<th colspan="13">Average Daily (Per Personnel)</th>
						...
						foreach ($averageReport as $averageReportKey => $averageReportValue) {
							$output .= '<tr>';
								...
								$output .= '<td>'.$averageReportValue["total_job"].'</td>
