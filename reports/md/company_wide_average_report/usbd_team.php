...
?>
	<script>
		$(document).ready(function(){
			var usbdTeamReport = $(".usbd-team-report").DataTable({
			    dom: "Bfrtip",
			    "bPaginate": false,
			    "bFilter": false,
			    bInfo: false,
	...
	if ($_POST) {
		...
		foreach ($dateRange as $dateRangeKey => $dateRangeValue) {
			...
			$joinedUserQUERY = mysqli_query($allConn, "SELECT
				e.id AS user_id,
			    LOWER(CONCAT(TRIM(e.first_name),' ',TRIM(e.last_name))) AS user_name,
			    DATE_FORMAT(u.created, '%Y-%m-%d') AS date_of_joining
			FROM
				vtechhrm.employees AS e
				LEFT JOIN vtechhrm.users AS u ON u.employee = e.id
			    LEFT JOIN vtechhrm.employeeprojects AS ep ON ep.employee = e.id
			WHERE
				ep.project = '6'
			AND
				DATE_FORMAT(u.created, '%Y-%m-%d') BETWEEN '$startDate' AND '$endDate'
			GROUP BY e.id");
		...
		if (array_sum(array_column($totalReport, "total_personnel")) == 0) {
			...
			$output = '<div class="row">
				...
				<div class="col-md-12">
					<table class="table table-striped table-bordered usbd-team-report">
						<thead>
