...
?>
	<script>
		$(document).ready(function(){
			var bdgTeamReport = $(".bdg-team-report").DataTable({
			    dom: "Bfrtip",
			    "bPaginate": false,
			    "bFilter": false,
			    bInfo: false,
	...
	if ($_POST) {
			...
			if (mysqli_num_rows($mainRESULT) > 0) {
				while ($mainROW = mysqli_fetch_array($mainRESULT)) {
					$totalReport[] = array(
						...
						"total_won" => $mainROW["total_shared_won"] + 0,
					...
					$averageReport[] = array(
						...
						"total_won" => round((($mainROW["total_shared_won"] / $mainROW["total_personnel"]) / $totalDays), 3),
		...
		if (array_sum(array_column($totalReport, "total_personnel")) == 0) {
			...
			$output = '<div class="row">
				...
				<div class="col-md-12">
					<table class="table table-striped table-bordered bdg-team-report">
						<thead>
